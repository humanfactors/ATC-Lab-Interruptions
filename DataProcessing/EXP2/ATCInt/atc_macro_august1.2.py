...
filenames = []
for file in os.listdir('Interruption CSV/'):
    if file.startswith("participant"):
        filenames.extend([os.path.join("Interruption CSV/",file)])
...
inputheaders = []
with open('Interruption CSV/participant_01_Bullpup_1.xml.csv', "rb") as h:
...
#Create blank accept and handoff variables. Then open data files for accept/handoff data
    ...
    handoffmisses = 0
    for time in (i[11] for i in handoff):
        if time == "":
