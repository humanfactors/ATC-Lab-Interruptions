...
# Results matrix
allResults = as.data.frame(matrix(,nrow=1,ncol = 365))

# get the participant number
part = as.numeric(gsub("[^0-9]+","",thisFile))
...
# get interruption type: 0 = none, 1 = Interruption, 2 = NBack
testTypeChar = as.character(rawdata[grep("Interruption Type",rawdata[,2])+1,2])

k = sub("Interruption","1",testTypeChar)
k = sub("NBack","2",k)
k = gsub("[^12]","0",k)
k[which(is.na(k))]="0"
testTypeNum = data.frame(as.numeric(k))
...
for (v in 1:3) { # PreIntCDT, PostIntCDT, OverlapCDT
  for (r in 1:5) {
    ...
    CDT[r,v] = mean(as.numeric(resConf[resConf[,1] == Nback[r] & !is.na(resConf[,7]) & resConf[,6] == typeCDT[v],7]))
    ...
    # Misses
    missCDT[r,v] = sum(as.numeric(resConf[resConf[,1] == Nback[r] & !is.na(resConf[,7]) & resConf[,6] == typeCDT[v],8]))
  ...
  # Conflict Detection Times
  CDT[16,v] = mean(as.numeric(resConf[as.numeric(resConf[,1]) %in% Nback & !is.na(resConf[,7]) & resConf[,6] == typeCDT[v],7]))
  ...
  # Misses
  missCDT[16,v] = mean(as.numeric(resConf[as.numeric(resConf[,1]) %in% Nback & !is.na(resConf[,7]) & resConf[,6] == typeCDT[v],8]))
...
for (r in 1:5) {
  confFA[r,1] = length(which(is.na(allConf[which(allConf[,1] == Nback[r]),5])))
...
confFA[16,1] = length(which(is.na(allConf[which(allConf[,1] %in% Nback),5]))) / 
  length(allConf[which(allConf[,1] %in% Nback),5])
...
# PM Dismiss
allResults[1,320:334] = c(dissPM[which(testTypeNum==2),1],dissPM[which(testTypeNum==1),1],dissPM[which(testTypeNum==0),1])
allResults[1,335:349] = c(dissRS[which(testTypeNum==2),1],dissRS[which(testTypeNum==1),1],dissRS[which(testTypeNum==0),1])

# Add Slope for ms calculations
allResults[1,350] = slope
...
headings = cbind(headings,"ms conversion","NBack NA",matrix("",nrow=1,ncol=14))
...
compiledResults = as.data.frame(matrix(,nrow=length(allFiles),ncol = 365))
